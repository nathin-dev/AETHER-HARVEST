...
Boss system or Infinite Harvest
"""

import pygame
import math 
import random 
from utils.constants import C_DANGER, C_ACCENT, C_WHITE, C_WHITE, WIDTH, HEIGHT
from utils.math_utils import normalize, dist, lerp
from engine.renderer import draw_glow_circle, draw_panel

BOSS_WAVE_INTERVAL = 5 # for every new wave a boss spawns
BOSS_TYPES = {
    "void_colossus": {
        "name": "VOID COLOSSUS",
        "color": (160, 50, 255),
...
class Boss:
    def __init__(self, x, y, boss_type):
        ...
        self.angle = 0.0
        self.hit_flash = 0.0
        self.spawn_anim = 0.0 # 0-1
        self.phase = 1 # 1, 2, 3
        self.orbit_ents = [] #orbiting minions
    ...
    def draw(self, surf, cam):
        ...
        # HP bar (wide top of screen when boss is active)
    ...
    def _draw_boss_bar(self, surf):
        ...
        name_surf = _pg_font.render(f"  {self.name}  HP{int(self.hp)}/{int(self.max_hp)}",
                                    True, C_WHITE)
        surf.blit(name_surf, (bar_rect.x + 8, bar_rect.y + 4))

# constant used in Boss.update 
BOSS_DASH_SPEED = 400

class BossManager:
    def __init__(self):
        self.boss = None 
        self.intro_timer = 0.0 # Dramatic pause before boss spawns
            ...
            if self.boss and self.boss.alive:
                ...
                if not self.boss.alive:
                    ...
                    particles.burst(self.boss.x, self.boss.y,
                                    self.boss_glow_col, count=80, speed=250, life=2.0)
