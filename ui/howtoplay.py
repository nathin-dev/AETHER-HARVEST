...
CONTROLS = [
    ...
    ("Ability 1",     "Q  —  Nova Burst  (explode all nearby ores)"),
    ("Ability 2",     "E  —  Void Pulse  (shockwave enemies)"),
    ("Ability 3",     "F  —  Aether Shield  (3s invincibility + heal)"),
    ("Save game",     "S"),
    ("Quit",          "Esc"),
]
...
MODES_INFO = [
    ...
    ("⚡ Blitz",        "5 minutes. Ores worth 2×. Enemies 40% faster."),
    ("☠ Void Siege",   "No upgrades. Pure skill. 4× ore value. Brutal."),
    ...
    ("✦✦ Hard",        "160% enemy HP, 1.5× DMG. High XP rewards."),
    ("✦✦✦ Nightmare",  "250% enemy HP, 2.2× DMG. +2.5× XP. Brutal."),
]
...
class HowToPlayScreen:
    ...
    def draw(self, surf):
        ...
        xs = font_md.render("✕", True, C_DANGER)
    ...
    def _draw_ores(self, surf, font_md, font_sm, font_xs, y0):
        y = y0 + 16
        hdr = font_md.render("Crystal Ore Types", True, C_WHITE)
        surf.blit(hdr, (80, y)); y += 40
    ...
    def _draw_upgrades(self, surf, font_md, font_sm, font_xs, y0):
        y = y0 + 10
        hdr = font_md.render("Upgrade Panel  (right side of screen — scroll to see all)",
                              True, C_WHITE)
    ...
    def _draw_modes(self, surf, font_md, font_sm, font_xs, y0):
        y = y0 + 16
        hdr = font_md.render("Game Modes", True, C_WHITE)
        surf.blit(hdr, (80, y)); y += 36

        for name, desc in MODES_INFO:
            if not name:
                pygame.draw.line(surf, (40,30,70), (80,y), (WIDTH-80,y), 1)
                if y < HEIGHT//2:
                    hdr2 = font_md.render("Difficulty Levels", True, C_WHITE)
                    surf.blit(hdr2, (80, y+4))
                y += 24; continue
            card = pygame.Rect(80, y, WIDTH-160, 44)
            draw_panel(surf, card, color=(16,12,36), border=(40,32,70),
                       alpha=200, radius=8)
            nm_s = font_sm.render(name, True, C_ACCENT)
            surf.blit(nm_s, (card.x+12, card.y+5))
            ds_s = font_xs.render(desc, True, C_GRAY)
            surf.blit(ds_s, (card.x+12, card.y+24))
            y += 50
