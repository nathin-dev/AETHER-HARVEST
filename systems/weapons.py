...
WEAPONS = {
    ...
    "crystal_beam": {
        "name": "Crystal Beam",
        "desc": "Slow heavy shot that chains to 3 enemies.",
        "icon": "◈",
        "color": (60, 220, 200),
        "glow": (100, 255, 240),
        "cooldown": 0.8,
...
class Projectile:
    __slots__ = ("x","y","vx","vy","damage","color","glow","radius","life","alive","pierce","hits")

    def __init__(self, x, y, dx, dy, damage, speed, color, glow, radius=5, life=1.8, pierce=0):
    ...
    def draw(self, surf, cam):
        ...
        for i in range(1, 4):
            ...
            tr = max(1, r - i)
            gs = pygame.Surface((tr*2+2, tr*2+2), pygame.SRCALPHA)
...
class WeaponSystem:
    ...
    def shoot(self, px, py, tx, ty, upgrade_level=0, damage_mult=1.0):
        ...
        if w["pattern"] == "single":
            self.projectiles.append(Projectile(px, py, tx-px, ty-py, dmg, w["speed"], w["color"], w["glow"], w["proj_size"], w["proj_life"], prc))
            ...
            for i in range(count):
                ...
                dx = math.cos(a)
                dy = math.sin(a)
                self.projectiles.append(Projectile(px, py, dx, dy, dmg, w["speed"] * random.uniform(0.8,1.0), w["color"], w["glow"], w["proj_size"], w["proj_life"], prc))
        ...
        for proj in self.projectiles:
            ...
            for enemy in enemies:
                ...
                if dist(proj.x,proj.y,enemy.x,enemy.y) < proj.radius + enemy.size:
                    ...
                    particles.burst(proj.x, proj.y, proj.glow, count=6, speed=80, life=0.3, size=3)
                    proj.hits += 1
    ...
    def draw_hud(self, surf, font_sm, font_xs):
        ...
        name_s = font_xs.render(w["name"], True, (220,215,245))
