...
Killing enemies and bosses grants XP. Levelling up gives stat bonuses
and eventually unlocks passive perks.
"""
import math
...
class XPSystem:
    ...
    def add_xp(self, amount, from_boss=False):
        """Grant XP. Returns True if levelled up."""
        if from_boss:
            amount = int(amount * 3)
        self.xp       += amount
    ...
    def register_kill(self, reward, is_boss=False):
        """Call when an enemy/boss dies. Returns XP gained."""
        self.kills += 1
    ...
    def _apply_perk(self, level):
        if level in LEVEL_PERKS:
            name, desc = LEVEL_PERKS[level]
            self.pending_perks.append((name, desc))
    ...
    def apply_to_player(self, player):
        """Sync level bonuses → player stats every frame."""
        if self.bonus_hp > 0:
    ...
    def draw(self, surf, font_md, font_sm, font_xs):
        ...
        if self.level_up_anim > 0:
            ...
            if self.pending_perks:
                ...
                p1 = font_sm.render(f"{name}", True, C_SUCCESS)
