"""
Combo system for Infinite Harvest
"""
import math
from utils.math_utils import lerp, smooth_step 

COMBO_DECAY = 2.5 # seconds untill combo resets
COMBO_STEPS = [1, 2, 3, 5, 8, 13, 20] # combo thresholds
...
class ComboSystem:
    def __init__(self):
        ...
        self.timer = 0.0
        self.multiplier = 1.0
        self.display_mult = 1.0  # smoothed for UI
        self.flash = 0.0    # flash anim on new tier
        self._prev_tier = 0
    ...
    def register_collect(self):
        """Call whenever player collects on ore. Returns current multiplier."""
        self.count += 1
        self.timer = 0.0
        new_tier = r = self.tier 
        if new_tier > self._prev_tier:
            self.flash = 0.6
            self._prev_tier = new_tier 
        self.multiplier = COMBO_MULTS[min(self._prev_tier, len(COMBO_MULTS) -1)]
    ...
    def draw(self, surf, font_md, font_xs):
        ...
        # Multiplier lablel
        mult_txt = f"✦ {self.display_mult:.2f}x"
