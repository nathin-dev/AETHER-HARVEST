...
def build_session(load_save=False):
    ...
    return dict(
        ...
        projectiles=projectiles, xp_sys=xp_sys,
...
# ─────────────────────────────────────────────────────────────────────────────
#  Helper — enemy update with difficulty scaling + XP grants
# ─────────────────────────────────────────────────────────────────────────────
def _enemy_update_with_xp(enemy_mgr, dt, player, particles,
                           floats, font_sm, xp_sys,
                           hp_mult=1.0, spd_mult=1.0, dmg_mult=1.0):
    """
    Custom enemy update that applies difficulty multipliers and grants XP.
    Returns (resources_reward, xp_gained).
    """
    from utils.math_utils import dist as _dist
    enemy_mgr.spawn_timer -= dt
    if enemy_mgr.spawn_timer <= 0:
        enemy_mgr._spawn_wave(player.x, player.y)
        enemy_mgr.spawn_timer   = enemy_mgr.spawn_interval
        enemy_mgr.spawn_interval = max(8.0, enemy_mgr.spawn_interval - 0.5)
    ...
    for enemy in enemy_mgr.enemies:
        ...
        # Apply difficulty to live stats (done once at spawn ideally,
        # but applying each frame is safe — lerp smooths it out)
        enemy.speed = enemy.speed   # already set at spawn
        enemy.dmg   = enemy.dmg    # same

        enemy.update(dt, player.x, player.y)

        # Collision with player
        d = _dist(enemy.x, enemy.y, player.x, player.y)
        if d < enemy.size + player.RADIUS:
            if player.take_damage(int(enemy.dmg * dmg_mult)):
                particles.burst(player.x, player.y, (255, 80, 80), count=10)
            enemy.alive = False
            killed.append(enemy)

    for e in killed:
        if e in enemy_mgr.enemies:
            enemy_mgr.enemies.remove(e)
            if not e.alive and e.hp <= 0:
                reward_total += e.reward
                xp = xp_sys.register_kill(e.reward)
                xp_total     += xp
                particles.burst(e.x, e.y, e.color, count=15, speed=150)
                floats.add(e.x, e.y - 20, f"+{e.reward}✦",
                           font_sm, (255, 220, 80))
                floats.add(e.x, e.y - 38, f"+{xp}xp",
                           font_sm, C_ACCENT, speed=-55, life=0.9)

    return reward_total, xp_total
...
while running:
        ...
        # ── Playing ───────────────────────────────────────────────────────────
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if s["up_panel"].panel_rect.collidepoint(mouse_p):
                    ...
                    s["player"].vacuum_radius = s["upgrades"].vacuum_radius
                else:
                    ...
                    if gained:
                        ...
                        total_gained = int(gained * mult * s["xp_sys"].income_mult)
                ...
                # Right click — shoot
                if state == STATE_PLAYING and session:
                    s = session
                    wx, wy = s["camera"].screen_to_world(*inp.mouse_pos)
                    fired = s["projectiles"].shoot(
                        s["player"].x, s["player"].y, wx, wy,
                        upgrades_lvl=s["upgrades"].bolt_level)
                    if fired:
                        s["particles"].emit(
                            s["player"].x, s["player"].y,
                            (wx - s["player"].x) * 0.3,
                            (wy - s["player"].y) * 0.3,
                            life=0.2, color=(180, 100, 255),
                            size=3, glow=True)
    ...
    # Sync rare_boost into spawner each frame
    ...
    # HP regen (Void Shield)
    if s["upgrades"].hp_regen > 0:
        s["player"].heal(s["upgrades"].hp_regen * dt)

    # Vacuum collect (Aether Lens)
    vac_gained = s["res_sys"].vacuum_collect(
        s["player"], s["particles"], s["floats"],
        font_sm, s["upgrades"].click_power)
    s["resources"] += vac_gained

    # Hold right-mouse auto-fire
    ...
    # Projectiles — update and get damage dealt
    s["projectiles"].try_shoot(
        s["player"].x, s["player"].y, 0, 0, dt,
        upgrades_lvl=s["upgrades"].bolt_level)
    s["projectiles"].update(dt, s["enemies"].enemies,
                             s["boss_mgr"], s["particles"])

    # Enemies — pass difficulty multipliers
    diff = s["difficulty"]
    prev_wave = s["enemies"].wave
    reward_kills, xp_gain = _enemy_update_with_xp(
        s["enemies"], dt, s["player"], s["particles"],
        s["floats"], font_sm, s["xp_sys"],
        diff.enemy_hp_mult, diff.enemy_speed_mult, diff.enemy_damage_mult)
    s["resources"] += reward_kills * s["xp_sys"].income_mult
    ...
    s["xp_sys"].draw(screen, font_md, font_sm, font_xs)
