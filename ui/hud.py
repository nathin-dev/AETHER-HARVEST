...
class UpgradePanel:
    ...
    def _draw_btn(self, surf, i, u, resources, upgrades):
        ...
        if maxed:
            ...
            bg = (25 + bg_pulse // 2, 25, 55 + bg_pulse)
        ...
        desc_lines = wrap_text(u["desc"], self.font_xs, rect.width - 60)
        for j, line in enumerate(desc_lines[:2]):
            desc_surf = self.font_xs.render(line, True, C_GRAY)
            surf.blit(desc_surf, (rect.x + 46, rect.y + 26 + j * 14))
...
class HUD:
    ...
    def _draw_hp_bar(self, surf, player):
        ...
        hp_txt = self.font_xs.render(
            f"HP  {int(player.hp)}/{player.max_hp}", True, C_WHITE
        )
