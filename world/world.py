...
World / background for Infinite Harvest
"""
...
class World:
    def __init__(self):
        ...
        # Pre-render background tile (repeated grid cell)
        self._bg_title = self._build_bg_title()
        self._nebula_s = self._build_nebula_surface()

    def _build_bg_title(self):
        title = pygame.Surface((GRID, GRID))
        title.fill(C_BG)
        # Subtle grid lines
        pygame.draw.line(title, (20, 16, 45), (0, 0), (GRID, 0), 1)
        pygame.draw.line(title, (20, 16, 45), (0, 0),(0,GRID), 1)
        return title
    
    def _build_nebula_surface(self):
        s = pygame.Surface((WORLD_W, WORLD_H), pygame.SRCALPHA )
        for (x, y, r, c) in self.nebulae:
            # Layered circles for soft glow 
    ...
    def draw(self, surf, cam):
        ...
        for ty in range(start_y, HEIGHT + GRID, GRID):
            for tx in range(start_x, WIDTH + GRID, GRID):
                surf.blit(self._bg_title, (tx, ty))
        ...
        # Stars (drawn per star for parallax)
