...
Contains - Visual only effects that react to game state - storm, pulse rings, void drifts , crystal showers.
"""
...
class VoidRift:
    ...
    def draw(self, surf, cam):
        ...
        if sx < -100 or sx > WIDTH+100 or sy < -100 or sy > HEIGHT+100:
            return 
        alpha = int(180 * min(1.0, self.life/self.max_life) * min(1.0, (self.max_life.life+0.5)/0.5))
        pulse = abs(math.sin(self.pulse))
        w = max(2, int(self.w * (0.6 + pulse*0.4)))

        s = pygame.Surface((w*2+4, self.h+4), pygame.SRCALPHA)
        
        # -  Ellipse for rift shape - #
        pygame.draw.ellipses(s, (180,60,255,alpha//3), (0,00,w*2+4, self.h+4))
        pygame.draw.ellipses(s, (120,40,200,alpha), (w-2,2, 8, self.h))
...
class CrystalShard:
    ...
    def __init__(self, cam_x, cam_y):
        ...
        self.rot_spd = random.randint(-3, 3)
    ...
    def draw(self, surf, cam):
        ...
        surf.blit(gs, (sx-s*2, sy-s*2), special_flags=pygame.BELND_ADD)
...
class PulseRing:
    """
    Expanding ring used for shock waves, level-ups, boss spawns.
    """
...
class WeatherSystem:
    def __init__(self):
        ...
        self._storm_active = False # crystal storm node 
    ...
    def update(self, dt, cam):
        ...
        # Crystal shards during storm 
        if self.storm_active:
            self._shard_timer -= dt 
            if self._shard_timer <= 0:
                self.shards_timer = random.uniform(0.03, 0.12)
        else:
            self._shard_timer = 0.0 
