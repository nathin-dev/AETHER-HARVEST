...
Edge warning arrows for Infinite Harvest.
...
MARGIN = 28 # px from edge 
MAX_ARR = 8 # max arrows shown at once 

def draw_warning_arrows(surf, player,enemies, boss_mgr, cam, font_xs):
    """Draw edge arrows pointing toward off-screen threads."""
    cx = WIDTH // 2
    cy = HEIGHT // 2

    threats = []

    # Collect off-screen enemies
    for e in enemies:
        if not e.alive:
            continue
    ...
    for (wx, wy, color, size) in threats:
        # Direction from screen center to threat
        dx = wx - player.x 
        dy = wy - player.y 
        ...
        # Find t where ray from center hits screen boundary 
        if nx != 0:
